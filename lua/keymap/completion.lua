...
local plug_map = {
	["n|<leader>fm"] = map_cmd("<Cmd>Format<CR>"):with_noremap():with_desc("Formater: Toggle format on save"),
}
...
function mapping.lsp(buf)
	local map = {
		...
		["n|<leader>sd"] = map_cr("Lspsaga show_line_diagnostics ++unfocus")
			:with_buffer(buf)
			:with_desc("lsp: Line diagnostic"),
		["n|gs"] = map_callback(function()
			vim.lsp.buf.signature_help()
		end):with_desc("lsp: Signature help"),
		-- ["n|rn"] = map_cr("Lspsaga rename"):with_buffer(buf):with_desc("lsp: Rename in file range"),
		["n|<leader>rn"] = map_cr("Lspsaga rename ++project"):with_buffer(buf):with_desc("lsp: Rename in project range"),
		["n|D"] = map_cr("Lspsaga hover_doc"):with_buffer(buf):with_desc("lsp: Show doc"),
